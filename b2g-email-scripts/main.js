// bleach setup, stolen from htmlchew, e.g. https://github.com/mozilla-b2g/gaia-email-libs-and-more/blob/master/data/lib/mailapi/htmlchew.js
var LEGAL_TAGS = [
...
// method to render things in iframe, stolen from
//https://github.com/mozilla-b2g/gaia/blob/afde38e7be5455d5d634c58a60a4d8009e0e7034/apps/email/js/iframe_shims.js
function renderIframe(htmlString, sanitizerName) {
  
  var funcName = sanitizerName || '';
  ...
  iframe.contentDocument.write('</head><body><h3>' + funcName + '</h3>');
...
require(["b2g-email-scripts/bleach.js/lib/bleach"], function(bleach) {


  // from htmlchew again
  sanitizers = {
    // as used in different settings from the email app

    'wrapTextIntoSafeHTMLString': function wrapTextIntoSafeHTMLString(text, wrapTag, transformNewlines, attrs) {
      ...
      'sanitizeAndNormalizeHtml': function sanitizeAndNormalizeHtml(htmlString) {
        return bleach.clean(htmlString, BLEACH_SETTINGS);
      },
      'generateSnippet': function generateSnippet(htmlString) {
          return bleach.unescapeHTMLEntities(bleach.clean(htmlString, BLEACH_SNIPPET_SETTINGS));
      },
  };

  // re-defining the whole filters thing
  filters = [
    [sanitizers.wrapTextIntoSafeHTMLString, "b2g-email::wrapTextIntoSafeHTMLString"],
    [sanitizers.sanitizeAndNormalizeHtml, "b2g-email::wrapTextIntoSafeHTMLString"],
    [sanitizers.generateSnippet, "b2g-email::wrapTextIntoSafeHTMLString"]
